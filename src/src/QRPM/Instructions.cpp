...
SymbolicGate::GateLabel SymbolicGate::parse_gate(const char* name) {
  if (str_equal_ci(name, "h")) {
    ...
    throw std::runtime_error(std::format("Error: unknown gate {}.", name));
  }
}
...
std::string PauliRotationGate::label() const {
  return std::format("R({}){}", pauli, adj ? "d" : "");
}
...
std::shared_ptr<Gate> parse_gate(const std::string& s, const Qubits& qubits) {
  if (s == "H" || s == "h") {
    ...
    throw std::invalid_argument(std::format("Invalid gate type: {}", s));
  }
}
...
std::string FreeFermionGate::label() const {
  std::string suffix = t ? "" : std::format("({:.3f})\n", t.value());
  return std::format("MG{}", suffix);
}
...
std::string FreeFermionGate::to_string() const {
  ...
  for (const auto& term : terms) {
    s.push_back(std::format("{:.3f} c_{}{} c_{}", term.a, term.i, term.adj ? "^dag" : "", term.j));
  }
...
: qubits(qubits), pauli(pauli), outcome(outcome) {
  ...
  if (qubits.size() != p.num_qubits) {
    throw std::runtime_error(std::format("Invalid number of qubits {} passed to measurement of pauli {}.", qubits, pauli.value()));
  }

  if (!p.hermitian()) {
    throw std::runtime_error(std::format("Cannot perform measurement on non-Hermitian Pauli string {}.", p));
  }
...
WeakMeasurement::WeakMeasurement(const Qubits& qubits, std::optional<double> beta, std::optional<PauliString> pauli, std::optional<bool> outcome)
  ...
  PauliString p = pauli ? pauli.value() : PauliString("+Z");
  if (qubits.size() != p.num_qubits) {
    throw std::runtime_error(std::format("Invalid number of qubits {} passed to weak measurement of pauli {}.", qubits, pauli.value()));
  }
}
