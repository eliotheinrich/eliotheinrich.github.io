...
binary_word BitString::to_integer() const {
  if (bits.size() > 1) {
    throw std::runtime_error(std::format("Cannot convert a bitstring containing more than {} bits.", binary_word_size()));
  }
...
BitString BitString::from_bits(size_t num_bits, binary_word bits) {
  if (num_bits >= binary_word_size()) {
    throw std::runtime_error(std::format("Cannot create a >{} BitString from a {}-bit integer.", binary_word_size(), sizeof(bits)));
  }
...
BitString BitString::operator^(const BitString& other) const {
  if (size() != other.size()) {
    throw std::runtime_error(std::format("Tried to perform ^ on BitStrings of unequal length: {} and {}", size(), other.size()));
  }
...
BitString BitString::superstring(const std::vector<uint32_t>& sites, size_t new_num_bits) const {
  if (sites.size() != num_bits) {
    throw std::runtime_error(std::format("When constructing a superstring bitstring, provided sites must have same size as num_qubits."));
  }
...
PauliString::PauliString(const std::string& paulis) : PauliString(process_pauli_string(paulis)) {
  ...
  for (size_t i = 0; i < num_qubits; i++) {
    if (s[i] == 'I') {
      ...
      std::cout << std::format("character {} not recognized\n", s[i]);
      throw std::runtime_error(std::format("Invalid string {} used to create PauliString.", paulis));
    }
  }
}
...
PauliString PauliString::superstring(const Qubits& qubits, size_t new_num_qubits) const {
  if (qubits.size() != num_qubits) {
    throw std::runtime_error(std::format("When constructing a superstring Pauli, provided sites must have same size as num_qubits. P = {}, qubits = {}.", to_string_ops(), qubits));
  }
...
PauliString PauliString::operator*(const PauliString& other) const {
  if (num_qubits != other.num_qubits) {
    throw std::runtime_error(std::format("Multiplying PauliStrings with {} qubits and {} qubits do not match.", num_qubits, other.num_qubits));
  }
...
void PauliString::evolve(const QuantumCircuit& qc) {
  ...
  if (qc.get_num_qubits() != num_qubits) {
    throw std::runtime_error(std::format("Cannot evolve a Paulistring with {} qubits with a QuantumCircuit with {} qubits.", num_qubits, qc.get_num_qubits()));
  }
    ...
    std::visit(quantumcircuit_utils::overloaded{
      [this](std::shared_ptr<Gate> gate) { 
        ...
        if (name == "H") {
          ...
          throw std::runtime_error(std::format("Invalid instruction \"{}\" provided to PauliString.evolve.", name));
        }
      },
...
bool PauliString::commutes(const PauliString& p) const {
  if (num_qubits != p.num_qubits) {
    throw std::invalid_argument(std::format("p = {} has {} qubits and q = {} has {} qubits; cannot check commutation.", p.to_string_ops(), p.num_qubits, to_string_ops(), num_qubits));
  }
