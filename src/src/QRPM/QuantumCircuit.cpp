...
void QuantumCircuit::validate_instruction(const Instruction& inst) const {
  ...
  auto validate_qubits = [num_qubits](const Qubits& qubits) {
    for (const auto q : qubits) {
      if (q >= num_qubits) {
        throw std::runtime_error(std::format("Invalid qubit {} passed to QuantumCircuit with {} qubits.", q, num_qubits));
      }
    }
  };
  ...
  auto validate_bits = [num_cbits](const Qubits& bits) {
    for (const auto b : bits) {
      if (b >= num_cbits) {
        throw std::runtime_error(std::format("Invalid bit {} passed to QuantumCircuit with {} classical bits.", b, num_cbits));
      }
    }
  };
  ...
  std::visit(quantumcircuit_utils::overloaded {
    ...
    [&](const ConditionedInstruction& cinst) {
      ...
      if (cinst.control && cinst.control.value() >= num_cbits) {
        throw std::runtime_error(std::format("Invalid control bit {} passed to QuantumCircuit with {} cbits.", cinst.control.value(), num_cbits));
      }

      if (cinst.target && cinst.target.value() >= num_cbits) {
        throw std::runtime_error(std::format("Invalid target bit {} passed to QuantumCircuit with {} cbits.", cinst.target.value(), num_cbits));
      }
    }
...
QuantumCircuit QuantumCircuit::bind_measurement_outcomes(const std::vector<bool>& outcomes) const {
  ...
  if (outcomes.size() != num_measurements) {
    throw std::runtime_error(std::format("Passed {} measurement outcomes to a circuit with {} measurements.", outcomes.size(), num_measurements));
  }
...
MeasurementVariant QuantumCircuit::get_measurement(size_t i) const {
  if (i >= measurement_map.size()) {
    throw std::runtime_error(std::format("Cannot retrieve measurement {} in a circuit with {} measurements.", i, measurement_map.size()));
  }
