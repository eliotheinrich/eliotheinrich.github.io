...
void QuantumState::validate_qubits(const Qubits& qubits) const {
  for (const auto q : qubits) {
    if (q > num_qubits) {
      throw std::runtime_error(std::format("Instruction called on qubit {}, which is not valid for a state with {} qubits.", q, num_qubits));
    }
  }
}
...
void QuantumState::_evolve(const Eigen::MatrixXcd& gate, const Qubits& qubits) {
  ...
  if (r == c) {
    ...
    throw std::runtime_error(std::format("Invalid gate shape: {}x{}", r, c));
  }
}
